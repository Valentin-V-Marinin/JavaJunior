...
public class Cart <T extends Food>{
    ...
    public void atomicCardBalancing()
    {
        ...
        foodstuffs.stream().forEach(food -> {
            if (!proteins.get()) proteins.set(food.getProteins());
            if (!fats.get()) fats.set(food.getFats());
            if (!carbohydrates.get()) carbohydrates.set(food.getCarbohydrates());
        });
        ...
        market.getThings(Food.class).stream()
                .forEach(thing ->{
                    ...
                    {
                        proteins.set(thing.getProteins());
                        foodstuffs.add((T)thing);
                    };
                    if (!fats.get() && thing.getFats())
                    {
                        fats.set(thing.getFats());
                        foodstuffs.add((T)thing);
                    };
                    if (!carbohydrates.get() && thing.getCarbohydrates())
                    {
                        carbohydrates.set(thing.getCarbohydrates());
                        foodstuffs.add((T)thing);
                    };
                });
